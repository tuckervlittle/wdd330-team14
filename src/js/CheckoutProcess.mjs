...
export default class CheckoutProcess {
  ...
  async checkout() {
    ...
    try {
      ...
      // Show a detailed, user-friendly error message
      import('./utils.mjs').then(({ alertMessage }) => {
        let userMessage = "Sorry, we couldn't process your order. ";
        if (err.name === 'servicesError' && err.message?.message) {
          userMessage += `Reason: ${err.message.message}`;
        } else if (err.name === 'servicesError' && err.message) {
          userMessage += `Reason: ${JSON.stringify(err.message)}`;
        } else if (err.message) {
          userMessage += `Reason: ${err.message}`;
        } else {
